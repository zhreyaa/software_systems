...
int main() {
    ...
    if (child_pid == 0) {
        ...
        // Redirect the standard input (stdin) to read from the pipe using fcntl
        ...
        // Redirect the standard output (stdout) to write to the pipe using fcntl
        int flags = fcntl(pipe_fd[1], F_GETFD);
        flags &= ~FD_CLOEXEC; // Clear the FD_CLOEXEC flag
        fcntl(pipe_fd[1], F_SETFD, flags);

        // Set up the pipe for stdout
